...
from src.gatr_v111_onnx.layers.linear import EquiLinear
...
class QKVModule(nn.Module):
    ...
    def forward(
        ...
        # print("before reaarrange", qkv_mv.shape)
        qkv_mv = rearrange(
            qkv_mv,
        ...
        # print("after reaarrange", qkv_mv.shape)
...
class MultiQueryQKVModule(nn.Module):
    ...
    def forward(
        ...
        # print("before rearange qmv", q_mv.shape, self.config.num_heads,self.config.hidden_mv_channels)
        # q_mv1 = torch.reshape(q_mv, [q_mv.shape[0], self.config.hidden_mv_channels,self.config.num_heads,q_mv.shape[2]  ])
        # q_mv =  torch.permute(q_mv1, (2,0,1,3))
        # print("after qmv reshape permute", q_mv1.shape)
        q_mv = rearrange(
            q_mv,
        ...
        # q_mv = torch.zeros((8, q_mv.shape[0],16)).to(q_mv.device)
        # print("after qmv", q_mv.shape)
        ...
        # Same for scalars
        if q_s is not None:
            # print("before rearange", q_s.shape, self.config.num_heads,self.config.hidden_s_channels)
            q_s = rearrange(
                q_s,
            ...
            # q_s = torch.reshape(q_s, [q_s.shape[0],self.config.hidden_s_channels,  self.config.num_heads, ])
            # q_s =  torch.permute(q_s, (2,0,1))
            # print("after rearange", q_s.shape)
            # q_s = torch.zeros((8, q_s.shape[0],16)).to(q_mv.device)
