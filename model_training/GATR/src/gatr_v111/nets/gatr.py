...
class GATr(nn.Module):
    ...
    def forward(
        ...
        # reference_mv = self._construct_dual_reference(multivectors, attention_mask)
    ...
    # @staticmethod
    # def _construct_dual_reference(inputs: torch.Tensor, attention_mask=None) -> torch.Tensor:
    #     if attention_mask is not None:
    #         # compute per-event mean using the seqlens from the block diagonal mask
    #         seqlens = attention_mask.q_seqinfo.seqstart_py  # e.g. [0, 7152, 11951, 16074, 18423]
    #         means = []
    #         for i in range(len(seqlens) - 1):
    #             start, end = seqlens[i], seqlens[i + 1]
    #             event_mean = inputs[start:end].mean(dim=0, keepdim=True)  # (1, 1, 16)
    #             means.append(event_mean.expand(end - start, -1, -1))      # repeat for each hit
    #         return torch.cat(means, dim=0)  # (N_total, 1, 16)
    #     else:
    #         mean_dim = tuple(range(1, len(inputs.shape) - 1))
